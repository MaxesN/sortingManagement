...
export const Modal: FC<Props> = ({isModal, onItemCreate, onChangeIsModal}) => {
  ...
  return (
    <>
      {
        isModal && <div onClick={onChangeIsModal} className="tw3-w-full  tw3-px-4 tw3-z-50 tw3-fixed tw3-top-0 tw3-h-full tw3-bg-black/25">
          <div onClick={(e) => e.stopPropagation()} className="tw3-bg-white tw3-max-w-[1000px] tw3-relative tw3-top-[5%] tw3-mx-auto">
            <div className="tw3-p-6 tw3-bg-sky-900 tw3-flex tw3-justify-between">
              <p className="tw3-text-white tw3-text-2xl tw3-font-medium">Užduotis</p>
              <div onClick={onChangeIsModal} className="tw3-flex tw3-cursor-pointer">
              ...
              <form className="tw3-text-sm tw3-font-semibold">
                <div className="tw3-flex">
                  <div className="tw3-w-[120px] tw3-mt-2">
                    ...
                    <p className={`${form.files.length > 0 ? 'tw3-mb-28' : 'tw3-mb-20'}`}>Failai</p>
                    <p>Kartoti</p>
                  </div>
                  <div className="tw3-w-full">
                    <Input  value={form.task} placeholder=""  onChangeValue={(e) => setForm({...form, task: e.target.value})} type='text'  isValid={form.task.length > 5 || form.task.length === 0}  />
                    <Select  value={form.type} onChange={(e) => setForm({...form, type: e!})}  placeholder="" options={typeOptions} />
                    <Select value={form.category} onChange={(e) => setForm({...form, category: e!})} placeholder="" withIcon={true} classNameIcon='tw3-top-2 tw3-left-3'  image={glass} className="tw3-my-6" options={categoryOptions} />
                    <DatePicker withIcon={true} date={form.date} onChangeDate={(e) =>setForm({...form, date: e })}/>  
                    <Select value={form.createBy} onChange={(e) => setForm({...form, createBy: [e!]})} placeholder="" withIcon={true} classNameIcon='tw3-top-2 tw3-left-3'  image={glass} className="tw3-my-6" options={createByOptions} />
                    <DropZoneCustom file={form.files} onChangeFile={(e) => setForm({...form, files: e})} />
                    <Select value={form.responsible} onChange={(e) => setForm({...form, responsible: [e!]})} className="tw3-my-6" placeholder="" options={responsibleOptions}></Select>
